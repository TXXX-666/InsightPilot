...
GOAL_EVALUATOR_SYSTEM = """You are evaluating a long-running goal condition in InsightPilot. Your task is to evaluate the condition described in the user message. Judge whether the user-provided condition is met.
...
#
# The prompt skeleton, output format, stage suffixes, and INSIGHTPILOT.md-injection
...
def load_auto_mode_rules() -> dict:
    ...
    if _cached_rules is None:
        # insightpilot/ -> python/ -> repo root -> assets/
...
def build_classifier_system(rules: dict) -> str:
    ...
    Environment / HARD BLOCK / SOFT BLOCK / ALLOW sections. The user's INSIGHTPILOT.md
...
def _strip_reminder(s: str) -> str:
    """Strip the INSIGHTPILOT.md/date <system-reminder> block the agent prepends to the
    first user message. It is repo config, not a user turn — leaving it in the
    classifier transcript would double-inject INSIGHTPILOT.md and let stage 2 mistake
    repo config for in-turn user authorization. INSIGHTPILOT.md reaches the classifier
    ...
    for m in history:
        ...
        if role == "user":
            ...
            if isinstance(content, str):
                ...
                text = ""
            # Drop the prepended INSIGHTPILOT.md/date reminder, then JSON-encode with
...
def classifier_user_message(rules: dict, transcript: str, suffix: str, claude_md: str | None = None) -> str:
    """Classifier user message: the (untrusted) INSIGHTPILOT.md context, the transcript
    projection, then the stage-specific suffix. INSIGHTPILOT.md goes in this USER
