...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="insightpilot",
        description="InsightPilot — a real-time research and monitoring agent",
        add_help=False,
    )
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .insightpilot/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    ...
    if args.help:
        print("""
Usage: insightpilot [options] [prompt]
...
Options:
  ...
  --model, -m         Model to use (default: claude-opus-4-6, or INSIGHTPILOT_MODEL env)
...
Examples:
  insightpilot "Research recent AI application internship requirements with sources"
  insightpilot --auto "Compare competitor product updates and verify every claim"
  insightpilot --plan "Plan an evidence-first market investigation"
  insightpilot --max-cost 0.50 --max-turns 20 "Research ophthalmology AI policy changes"
  OPENAI_API_KEY=sk-xxx insightpilot --api-base https://aihubmix.com/v1 --model gpt-4o "hello"
  insightpilot --resume
  insightpilot  # starts interactive REPL
""")
        sys.exit(0)

    permission_mode = _resolve_permission_mode(args)
    model = args.model or os.environ.get("INSIGHTPILOT_MODEL", "claude-opus-4-6")
