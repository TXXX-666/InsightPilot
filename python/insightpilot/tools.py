...
from .research_tools import RESEARCH_TOOL_DEFINITIONS, execute_research_tool

# ─── Permission modes ──────────────────────────────────────

PermissionMode = str  # "default" | "plan" | "acceptEdits" | "bypassPermissions" | "dontAsk" | "auto"

READ_TOOLS = {"read_file", "list_files", "grep_search", "web_fetch", "web_search", "browser_open", "read_pdf", "read_docx", "read_spreadsheet", "query_evidence"}
EDIT_TOOLS = {"write_file", "edit_file"}
EXTERNAL_SIDE_EFFECT_TOOLS = {"schedule_monitor", "request_notification"}

# Concurrency-safe tools can run in parallel (read-only, no side effects)
CONCURRENCY_SAFE_TOOLS = {"read_file", "list_files", "grep_search", "web_fetch", "web_search", "browser_open", "read_pdf", "read_docx", "read_spreadsheet", "query_evidence"}
...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .insightpilot/skills/. Returns the skill's resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "type": {"type": "string", "enum": ["explore", "plan", "general", "search-planner", "web-researcher", "document-analyst", "data-analyst", "evidence-verifier", "critic", "report-writer", "monitor"], "description": "Specialized sub-agent type. Default: general"},
            },
...
def _web_fetch(inp: dict) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "insightpilot/1.0"})
    try:
...
# ─── Permission rules (.insightpilot/settings.json) ───────────────
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".insightpilot" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".insightpilot" / "settings.json")
...
def check_permission(
    ...
    if tool_name in EXTERNAL_SIDE_EFFECT_TOOLS:
        if mode == "dontAsk":
            return {"action": "deny", "message": f"Auto-denied external side effect: {tool_name}"}
        return {"action": "confirm", "message": f"external product action: {tool_name}"}
...
async def execute_tool(
    ...
    research_names = {tool["name"] for tool in RESEARCH_TOOL_DEFINITIONS}
    if name in research_names:
        return await execute_research_tool(name, inp)
